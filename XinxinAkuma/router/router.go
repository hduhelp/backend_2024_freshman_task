...
func InitRouter() *gin.Engine {
	...
	r.POST("/submit", auth.AuthMiddleware(), problem.Submit)                  //用户回答问题路由
	r.GET("/submit", auth.AuthMiddleware(), problem.GetSubmission)            //所有回答路由
	r.PUT("/update", auth.AuthMiddleware(), problem.Update)                   //更新问题
	r.POST("/search", auth.AuthMiddleware(), problem.SearchProblem)           //查询问题
	r.GET("/generateAianswer", auth.AuthMiddleware(), problem.GenerateAnswer) //查询ai答案
